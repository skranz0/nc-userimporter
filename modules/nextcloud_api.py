...
class NextcloudAPI:
    ...
    def create_groups(self, groups):
        ...
        return [
            self._request("POST", "ocs/v1.php/cloud/groups", data={"groupid": group})
            for group in groups
        ]
    ...
    def add_user_to_groups(self, username, groups):
        ...
        responses = [
            self._request(
                "POST",
                f"ocs/v1.php/cloud/users/{username}/groups",
                data={"groupid": group},
            )
            for group in groups
        ]
    ...
    def remove_user_from_groups(self, username, groups):
        ...
        for group in groups:
            response = self._request(
                "DELETE", f"ocs/v1.php/cloud/users/{username}/groups?groupid={group}"
            )
    ...
    def promote_user_in_group(self, username, groups):
        ...
        for group in groups:
            response = self._request(
                "POST",
                f"ocs/v1.php/cloud/users/{username}/subadmins",
                data={"groupid": group},
            )
            if not self.is_successful(response):
                self.logger.error(
    ...
    def demote_user_in_group(self, username, groups):
        ...
        return [
            self._request(
                "DELETE", f"ocs/v1.php/cloud/users/{username}/subadmins?groupid={group}"
            )
            for group in groups
        ]
