...
def load_language(config_reader):
    ...
    try:
        ...
        logging.error(
            f"Language file '{lang_file_path}' not found. Defaulting to English."
        )
        ...
        try:
            ...
            with open(
                default_lang_file_path, "r", encoding="utf-8"
            ) as default_lang_file:
                default_language_data = json.load(default_lang_file)
                logging.info(
                    f"Loaded default English language file: {default_lang_file_path}"
                )
                return default_language_data
        except FileNotFoundError:
            # If the default language file is also not found, raise a critical error
            logging.critical(
                f"Default language file '{default_lang_file_path}' not found. Exiting program."
            )
            raise SystemExit(
                f"Critical Error: Default language file '{default_lang_file_path}' not found."
            )
