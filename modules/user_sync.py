...
class NextcloudUserManager:
    ...
    def load_csv_users(self, csv_users):
        ...
        for user in csv_users:
            ...
            if not user.get("displayname"):
                logging.info(
                    f"Displayname for user '{user['username']}' is empty. Using username as displayname."
                )
    ...
    def fetch_and_populate_user_details(self, nextcloud_user):
        try:
            ...
            if self.api.is_successful(response):
                ...
                logging.error(
                    f"Failed to fetch details for user '{nextcloud_user['id']}'."
                )
        except Exception as e:
            logging.error(
                f"Error fetching details for user '{nextcloud_user['id']}': {e}"
            )
    ...
    def populate_user_details(self, nextcloud_user, response):
        ...
        logging.debug(
            f"Full API response for user {nextcloud_user['id']}: {etree.tostring(root, pretty_print=True).decode('utf-8')}"
        )

        nextcloud_user["displayname"] = self.api.get_xml_value(
            root, ".//displayname", ""
        )
        nextcloud_user["email"] = self.api.get_xml_value(root, ".//email", "")
        nextcloud_user["groups"] = self.api.get_user_groups(nextcloud_user["id"])
        nextcloud_user["subadmin"] = self.api.get_user_subadmin_groups(
            nextcloud_user["id"]
        )
    ...
    def detect_changes(self, csv_user, nextcloud_user):
        ...
        # Groups comparison
        csv_groups = set(
            group.strip() for group in csv_user["groups"].split(",") if group.strip()
        )
        ...
        csv_subadmin = set(
            group.strip() for group in csv_user["subadmin"].split(",") if group.strip()
        )
        nextcloud_subadmin = set(
            group.strip() for group in nextcloud_user.get("subadmin", [])
        )
    ...
    def compare_and_display_changes(self, csv_user, nextcloud_user):
        ...
        table_data = [
            [
                self.lang.get(
                    "user_sync_field", "Missing translation string for: user_sync_field"
                ),
                self.lang.get(
                    "user_sync_csv_user",
                ...
                self.lang.get(
                    "user_sync_nc_user",
                    "Missing translation string for: user_sync_nc_user",
                ),
            ],
            [
                self.lang.get(
                    "user_sync_username",
                    "Missing translation string for: user_sync_username",
                ),
                csv_user["username_mapped"],
                nextcloud_user["id"],
            ],
            [
                self.lang.get(
                    "user_sync_display_name",
                    "Missing translation string for: user_sync_display_name",
                ),
                csv_user["displayname"],
                nextcloud_user.get("displayname", ""),
            ],
            [
                self.lang.get(
                    "user_sync_email", "Missing translation string for: user_sync_email"
                ),
                csv_email,
                nextcloud_email,
            ],
            [
                self.lang.get(
                    "user_sync_groups",
                    "Missing translation string for: user_sync_groups",
                ),
                csv_user["groups"],
                ", ".join(nextcloud_user.get("groups", [])),
            ],
            [
                self.lang.get(
                    "user_sync_subadmin_groups",
                    "Missing translation string for: user_sync_subadmin_groups",
                ),
                csv_user["subadmin"],
                ", ".join(nextcloud_user.get("subadmin", [])),
            ],
        ]

        print("")
        print(
            f"{self.lang.get('user_sync_changes_detected', 'Missing translation string for: user_sync_changes_detected')}"
        )
        print(tabulate(table_data, headers="firstrow", tablefmt="grid"))

        apply_changes = input(
            f"{self.lang.get('user_sync_prompt_changes', 'Missing translation string for: user_sync_prompt_changes')}"
        ).lower()
    ...
    def check_for_deleted_users(self, csv_users, nextcloud_users):
        ...
        for username in deleted_users:
            ...
            nextcloud_user = next(
                user for user in nextcloud_users if user["id"] == username
            )
    ...
    def prompt_user_deletion(self, username):
        ...
        nextcloud_user = {
            "id": username,
            "displayname": self.api.get_xml_value(
                etree.fromstring(user_details_response["response"].content),
                ".//displayname",
                "",
            ),
            "email": self.api.get_xml_value(
                etree.fromstring(user_details_response["response"].content),
                ".//email",
                "",
            ),
        ...
        table_data = [
            ...
            [
                ...
                nextcloud_user["email"],
                nextcloud_user["groups"],
                nextcloud_user["subadmin"],
            ],
        ]
        print("")
        print(
            f"{self.lang.get('user_sync_prompt_delete', 'Missing translation string for: user_sync_prompt_delete')}"
        )
        print(tabulate(table_data, headers="firstrow", tablefmt="grid"))

        if (
            input(
                f"{self.lang.get('user_sync_prompt_deletion', 'Missing translation string for: user_sync_prompt_deletion')}"
            ).lower()
            == "y"
        ):
            ...
            if self.api.is_successful(response):
                logging.info(
                    f"{self.lang.get('user_sync_successful_deletion', 'Missing translation string for: user_sync_successful_deletion')}: '{username}'"
                )
            else:
